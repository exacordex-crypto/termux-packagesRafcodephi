...
Generated `.deb` files are collected by `scripts/ci/collect-package-artifacts.sh` into per-architecture `debs-<arch>-<commit>.tar` artifacts, while matching checksum files are uploaded separately. The collector validates the CI architecture contract and writes a small `debs-<arch>-<commit>.manifest` next to each archive so upload/debug paths can confirm the architecture, commit, archive path, checksum file, and number of packaged `.deb` files without unpacking the tarball. Downloaded artifacts are validated by `scripts/ci/validate-downloaded-package-artifacts.sh` before upload-test or production upload extracts them, so missing architecture archives, manifest drift, checksum-count mismatches, and corrupt tarballs fail before publication.
